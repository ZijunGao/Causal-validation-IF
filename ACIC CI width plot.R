...
for(i in 1:length(setting.seq)){
  ...
  plot.data = data.frame(width = c(unlist(lapply(record$record.absolute.total, function(x){mean(qnorm(1 - alpha / 2) * x$sd.plug.in[, 1])})), 
                                   ...
                                   unlist(lapply(record$record.absolute.total, function(x){mean(qnorm(1 - alpha / 2) * x$sd.semiEfficient[, 1])})),  
                                   unlist(lapply(record$record.absolute.total, function(x){mean(qnorm(1 - alpha / 2) * x$sd.plug.in[, 2])})), 
                                   unlist(lapply(record$record.absolute.total, function(x){mean(qnorm(1 - alpha / 2) * x$sd.AVDS[, 2])})), 
                                            unlist(lapply(record$record.absolute.total, function(x){mean(qnorm(1 - alpha / 2) * x$sd.semiEfficient[, 2])})),  
                                            unlist(lapply(record$record.relative.total, function(x){mean(qnorm(1 - alpha / 2) * x$sd.semiEfficient)})),  
                                            unlist(lapply(record$record.relative.total, function(x){mean(qnorm(1 - alpha / 2) * x$sd.AVDS)})),  
                                            unlist(lapply(record$record.relative.total, function(x){mean(qnorm(1 - alpha / 2) * x$sd.semiEfficient)}))))
