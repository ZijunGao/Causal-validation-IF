...
n = 400; n.train = n; n.test = 10000; d = 5; n.fold = 2 # 5; 2
...
for(i in 1 : m){
  ...
  # Oracle
  record$oracle.infData[i, ] = sapply(seq(1, length(lambda.seq)), function(x){mean((tau.hat.functions(X.test, x) - tau.test)^2)})
  
  record$oracle[i, ] = sapply(seq(1, length(lambda.seq)), function(x){mean((tau.hat.functions(X, x) - tau)^2)})
  
  # Semi-parametrically efficient
  temp.semiEfficient = sapply(seq(1, length(lambda.seq)), function(x){efficient.MSE(Y = Y, X = X, W = W, tau.hat.function = function(X){tau.hat.functions(X, x)}, folds = folds, nuisance.hat = nuisance.hat)})
  record$semiEfficient[i,] = unlist(temp.semiEfficient[1,])
  record$sd.semiEfficient[i,] = unlist(temp.semiEfficient[2, ])
  
  # AVDS
  temp.AVDS = sapply(seq(1, length(lambda.seq)), function(x){AVDS.MSE(Y = Y, X = X, W = W, tau.hat.function = function(X){tau.hat.functions(X, x)}, folds = folds, nuisance.hat = nuisance.hat)})
  record$AVDS[i,] = unlist(temp.AVDS[1, ])
  record$sd.AVDS[i,] = unlist(temp.AVDS[2, ])
  
  # Plug-in
  temp.plug.in = sapply(seq(1, length(lambda.seq)), function(x){plug.in.MSE(Y = Y, X = X, W = W, tau.hat.function = function(X){tau.hat.functions(X, x)}, folds = folds, nuisance.hat = nuisance.hat)})
...
matplot(lambda.seq, as.data.frame(result[c("oracle.infData", "oracle", "semiEfficient", "plug.in", "AVDS")]), type = "l", lwd = 2, lty = 1
        #, ylim = c(-20, 20)
)
lines(lambda.seq, result$semiEfficient + result$sd.semiEfficient, lty = 3, col = 3)
lines(lambda.seq, result$semiEfficient - result$sd.semiEfficient, lty = 3, col = 3)
lines(lambda.seq, result$plug.in + result$sd.plug.in, lty = 3, col = 4)
lines(lambda.seq, result$plug.in - result$sd.plug.in, lty = 3, col = 4)
lines(lambda.seq, result$AVDS + result$sd.AVDS, lty = 3, col = 5)
lines(lambda.seq, result$AVDS - result$sd.AVDS, lty = 3, col = 5)
legend("topleft", legend = c("oracle.infData", "oracle", "semiEfficient", "plug.in", "AVDS"), lwd = 3, lty = 1, col = seq(1, 10))
